...
RSpec.describe DonationFlow do
  before (:each) do
    @user = create(:user)
  ...
  describe '#save!' do
    context 'card information is not passed' do
      it 'raises an error' do
        ...
        donation_decorator = DonationFlow.new donation, @token_hash, @user, false
  ...
  describe '#subscribe_to_mail' do
    context 'when subscribe is true' do
      it 'adds user to mailing list' do
        ...
        donation = create(:donation)
        donation_decorator = DonationFlow.new donation, @token_hash, @user, true
